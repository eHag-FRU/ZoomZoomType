    ...
    <div className={`bg-transparent ${theme} icon-${theme} px-3 py-2`}>
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2">
        <div className="d-flex flex-wrap align-items-center gap-2">
          <Link className={`d-flex gap-2 me-2 nav-anchor-hover icon-${theme}`} to="/" style={{ textDecoration: "none" }}>
            <FontAwesomeIcon icon="fa-solid fa-car-on" style={{ fontSize: "1.5em" }} className={`icon-${theme}`} />
            <h5 className={`mb-0 nav-anchor-hover`}>Zoom Zoom Type</h5>
          </Link>
          <Link className="nav-anchor-visited nav-anchor-hover text-white" to="/games">
            <FontAwesomeIcon icon="fa-solid fa-keyboard" style={{ fontSize: "1.5em" }} className={`icon-${theme}`} />
          </Link>
          <Link className="nav-anchor-visited nav-anchor-hover text-white" to="/leaderBoard">
            <FontAwesomeIcon icon="fas fa-crown" style={{ fontSize: "1.5em" }} className={`icon-${theme}`} />
          </Link>
          <Link className="nav-anchor-visited nav-anchor-hover text-white" to="/about">
            <FontAwesomeIcon icon="fas fa-info-circle" style={{ fontSize: "1.5em" }} className={`icon-${theme}`} />
          </Link>
          <span className="ms-2 d-flex flex-row">
            Average WPM:&nbsp;
            <strong>
              <p id="highestUserWPM">{wpm}</p>
            </strong>
          </span>
        </div>
